...
- **Sagt dir, warum es sich langsam anfühlte.** Ohne Konsole gestartet, schreibt die App `data/app.log` (eine Zeitangabe je Zeile, beim Start gekürzt, sobald sie 2 MB übersteigt). Jedes Diktat hinterlässt dort eine `[timing]`-Zeile: Taste->Stopp, Stopp->Anzeige, Warten, Transkription, Aufräumen, gesamt. Raten über die Geschwindigkeit erübrigt sich damit, die Zahlen stehen in der Datei.
