...
Or double-click `testen-konsole.bat`: that runs the app with a visible console (`kalibrieren.bat` does the same for calibration). For every dictation the console shows the timings, whether the text was pasted or left on the clipboard, alias corrections, whether a faithfulness rule tripped and why, and the first 120 characters of the delivered text; a long dictation also shows the first 60 characters of every piece transcribed while you were still speaking. The raw transcript is not shown next to the cleaned text; to compare the two, `py whisperflow.py --clean-text "…"` (above) prints both for a text you type.
...
Started without a console (`pythonw`, the Startup shortcut, `start-whisperflow.bat`), the app also writes `data/app.log` for troubleshooting, and that file holds parts of what you dictate:

- for every finished dictation, the first 120 characters of its text as delivered (cleaned, translated or raw) and alias corrections such as `Nextclout->Nextcloud`;
- for a dictation longer than about 12 seconds (`audio.chunk_seconds`), also the first 60 characters of every piece transcribed while you are still speaking: roughly one line per 12 seconds of speech, in the language you spoke, even if the recording is discarded afterwards;
- the number, web or e-mail address or @handle that tripped a faithfulness rule;
- the whole text whenever it could not be placed on the clipboard (if it was appended to an earlier dictation, that one too);
- the text of a tray notification that could not be shown, which can be the first 90 characters of a dictation.

Whenever that file grows past 2 MB (checked at startup and after every line while the app keeps running), it is cut back to its newest whole lines, at most 1 MB; `ui.history_file: null` does not affect it. To empty it, quit the app (tray → Quit) and delete the file; the next start without a console creates a new one.

Outbound connections: the Whisper model is downloaded from Hugging Face once, on the first start, and again only when you switch `stt.model` to a model that is not downloaded yet (or to finish an interrupted download); after that it loads from the local cache. One exception: if the model's folder has no `tokenizer.json`, faster-whisper fetches the tokenizer from Hugging Face at every start; the "speech model" line of `py whisperflow.py --doctor` shows whether that applies on your machine. Apart from that, the only connection is the call to the language model at the address you set in `config.yaml`. With `127.0.0.1` there and `--doctor` reporting that the speech model loads without internet, no byte leaves the machine.
...
- **Sagt dir, warum es sich langsam anfühlte.** Ohne Konsole gestartet, schreibt die App `data/app.log` (eine Zeitangabe je Zeile; übersteigt sie 2 MB, beim Start oder während die App läuft, bleiben nur ihre jüngsten ganzen Zeilen, höchstens 1 MB). Jedes Diktat hinterlässt dort eine `[timing]`-Zeile: Taste->Stopp, Stopp->Anzeige, Warten, Transkription, Aufräumen, gesamt. Raten über die Geschwindigkeit erübrigt sich damit, die Zahlen stehen in der Datei. Die Datei enthält auch Teile des Diktierten, siehe Datenschutz unten.
...
Oder **`testen-konsole.bat`** doppelklicken: startet das Programm mit sichtbarer Konsole (**`kalibrieren.bat`** macht dasselbe für die Kalibrierung mit der eigenen Stimme). Die Konsole zeigt zu jedem Diktat die Zeiten, ob der Text eingefügt wurde oder in der Zwischenablage liegt, Alias-Treffer, ob ein Treue-Wächter angesprungen ist und warum, und die ersten 120 Zeichen des gelieferten Texts; ein langes Diktat zeigt außerdem die ersten 60 Zeichen jedes Abschnitts, der schon während des Sprechens erkannt wurde. Den Rohtext zeigt sie nicht neben dem bereinigten Text; zum Vergleich druckt `py whisperflow.py --clean-text "…"` (oben) beides für einen getippten Text.
...
Audio bleibt im Arbeitsspeicher und wird danach verworfen, es wird nichts aufgezeichnet. Der gelieferte Text steht 14 Tage lokal in `data/history.log`; alte Einträge fallen beim Start weg und, solange die App läuft, mit dem ersten Diktat jedes Tages (nicht laufend, und nicht, während sie nur untätig weiterläuft). `ui.history_file: null` schaltet das ab.

Ohne Konsole gestartet (`pythonw`, Autostart-Verknüpfung, `start-whisperflow.bat`), schreibt die App außerdem `data/app.log` zur Fehlersuche, und darin stehen Teile dessen, was du diktierst:

- zu jedem fertigen Diktat die ersten 120 Zeichen seines Texts, so wie er geliefert wurde (bereinigt, übersetzt oder roh), und Alias-Treffer wie `Nextclout->Nextcloud`;
- bei einem Diktat über rund 12 Sekunden (`audio.chunk_seconds`) außerdem die ersten 60 Zeichen jedes Abschnitts, der schon während des Sprechens erkannt wird: grob eine Zeile je 12 Sekunden Rede, in der Sprache, in der du gesprochen hast, auch wenn die Aufnahme danach verworfen wird;
- der Auslöser, wenn ein Treue-Wächter wegen einer Zahl, einer Web- oder Mail-Adresse oder eines @-Handles anspringt;
- der ganze Text, wenn er nicht in die Zwischenablage gelegt werden konnte (wurde er an ein früheres Diktat angehängt, auch dieses);
- der Text einer Tray-Meldung, die nicht angezeigt werden konnte; das können die ersten 90 Zeichen eines Diktats sein.

Wächst diese Datei über 2 MB (geprüft beim Start und, solange die App läuft, nach jeder Zeile), bleiben nur ihre jüngsten ganzen Zeilen, höchstens 1 MB; `ui.history_file: null` hat darauf keinen Einfluss. Leeren: App beenden (Tray → Beenden) und die Datei löschen; der nächste Start ohne Konsole legt sie neu an.

Ausgehende Verbindungen: Das Whisper-Modell wird einmal von Hugging Face heruntergeladen, beim ersten Start, und danach nur, wenn du `stt.model` auf ein Modell umstellst, das noch nicht heruntergeladen ist (oder um einen abgebrochenen Download zu beenden); danach kommt es aus dem lokalen Cache. Eine Ausnahme: Liegt im Ordner des Modells keine `tokenizer.json`, holt faster-whisper den Tokenizer bei jedem Start von Hugging Face; ob das auf deinem Rechner zutrifft, zeigt `py whisperflow.py --doctor` in der Zeile „speech model …“. Abgesehen davon gibt es nur den Aufruf des Sprachmodells unter der Adresse aus `config.yaml`. Steht dort `127.0.0.1` und meldet `--doctor`, dass das Whisper-Modell ohne Internet lädt, verlässt kein Byte den Rechner.
