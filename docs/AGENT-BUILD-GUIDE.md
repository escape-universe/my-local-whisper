...
| "It still is not reactive when I release the key" - while the measurement said key->stop 0 ms | The badge said **"listening"** during transcription, and a red dot and a thin progress ring look the same out of the corner of your eye. Nothing said *recording has stopped* | Say it, do not just do it: an unmistakable stop moment (grey square, "recording off") for ~600 ms, a short beep on release, and never a label that can be read as "still capturing". **"Measured fast" and "experienced fast" are two different findings** |
...
| A German keyboard loses `@ € | { }` | AltGr was used ALONE as the trigger; the OS sends Ctrl+Alt for it | Never use AltGr alone. A chord such as AltGr+Shift is fine: AltGr by itself still types those characters, only adding another key on top of it triggers the action |
